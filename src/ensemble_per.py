...
if __name__ == '__main__':
    ...
    for turn in range(1,turns+1):
        ...
        fname = f'ensemble_selection_test/ensemble_selection_PER_{turn}.csv'
        ...
        for lang1 in LANG:
            for i in test_set[lang1]:
                ...
                for model_num in K:
                    # print(f'K = {model_num}')
                    loss = [round(1/(1-x),4) for x in tmp_df['x'][:model_num]]
                    s = sum(loss)
                    weight = [x/s for x in loss]
                    # print(weight)
                    X_simple = np.zeros(len(y_test.index))
                    for cnt in range(model_num):
                        X_simple = X_simple + tmp_y_pred[cnt] * weight[cnt]
                        
                    y_simple = [int(x > 0.5) for x in X_simple]
